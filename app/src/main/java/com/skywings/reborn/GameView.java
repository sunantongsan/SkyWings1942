...
public class GameView extends View {
    ...
    public GameView(Context c){
        ...
        welcome=BitmapFactory.decodeResource(getResources(),R.drawable.galaxy1942_origin);
    }
    ...
    private void drawOrigin(Canvas c){
        ...
        if(welcome!=null)c.drawBitmap(welcome,null,new RectF(0,0,W,H),p); else c.drawColor(0xff030814);
    ...
    private void drawBase(Canvas c){
        int W=getWidth(),H=getHeight(); c.drawColor(0xff06101c);
        p.setShader(new LinearGradient(0,0,W,H,0xff0a2032,0xff102b1c,Shader.TileMode.CLAMP));c.drawRect(0,0,W,H,p);p.setShader(null);
    ...
    private void drawAssault(Canvas c){
        int W=getWidth(),H=getHeight();
        c.drawColor(0xff17311d);
        p.setStyle(Paint.Style.FILL);p.setColor(0xff2f6031);c.drawRect(0,0,W,H,p);
        p.setColor(0xff255b7a);c.drawRect(0,H*.63f,W,H,p);
